			...
			success:function(result){
				if(result.data==null){
					...
					for(var i=0;i<result.data.length;i++){
						...
						+"</td><td>"+sender+"</td><td>"+result.data[i].nickname+"</td><td><a onclick='UI.findMsgList(\""+sender+"\")'>"+result.data[i].count+"条</a></td><td>"+result.data[i].body+"</td><td><button onclick='UI.openMsg(\""+sender+"\")' class='layui-btn' style='height:30px;line-height:30px;border-radius:3px'>发消息</button></td></tr>";
					}
					$("#msg_td").empty();
					$("#msg_td").append(html);
				}
...
//					html+="<tr><td>暂无数据</td><td></tr>";
					...
					sum=result.data.length;
					for(var i=0;i<result.data.length;i++){
						html+="<tr><td>"+result.data[i].sender+"</td><td>"+result.data[i].nickname+"</td><td><a onclick='UI.findMsgList(\""+result.data[i].sender+"\")'>"
						+result.data[i].count+"条</a></td><td>"+result.data[i].body+"</td><td><button onclick='UI.openMsg(\""+result.data[i].sender+"\")' class='layui-btn' style='height:30px;line-height:30px;border-radius:3px'>发消息</button></td></tr>";
					}
					$("#newMsg_tb").empty();
...
//				});
	...
	pushOneToAll:function() {
		...
		alert(title);
		var sub=$("#pushbody2").val();
		var img=$("#pushbody3").val();
		var url=$("#pushbody4").val();
		$.ajax({
			url : '/mp/pushToAll',
			method : 'POST',
			data : {
				...
				if(result.data==null){
					...
					for(var i=0;i<result.data.length;i++){
						...
						html+="<tr><td><img width='40px' onerror='this.src=\"/pages/img/ic_avatar.png\"' src='"+myFn.getImgUrl(sender)+"'></td><td>"+sender+"</td><td>"
							+result.data[i].nickname+"</td><td>"+result.data[i].content+"</td><td><button onclick='UI.openMsg(\""+sender+"\")' class='layui-btn' style='height:30px;line-height:30px;border-radius:3px'>发消息</button></td></tr>";
					}
					$("#newMsg_body").empty();
