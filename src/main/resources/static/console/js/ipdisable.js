...
//    }
    ...
    var baseTable = table.render({
      ...
      ,cols: [[ //表头
          ...
          ,{field: 'cip', title: 'IP',sort:'true', width:100}
         
          ,{field: 'disable',title:'状态',width:100,templet: function(d){
        	  if(d.disable==1)
        		  return "禁用";
        	  else
        		  return "未禁用";
 ...
 /*   $(".nickName").val('');*/
  ...
  table.on('tool(body_list)', function(obj){
      ...
      }else if(layEvent === 'updisable'){// 禁用
    	  Base.updisable(obj.data,obj.data.id,1);
      }else if(layEvent === 'updisableback'){// 解禁    
    	  Base.updisable(obj.data,obj.data.id,-1);
      }
  });
...
var Base={
		...
		myFn.invoke({
			...
			success:function(result){
				...
				if(result.data!=null){
					...
					$("#cip").val(result.data.cip),
					$("#disable").val(result.data.disable)
				}
				$("#addConfigTitle").empty();
	...
	// 禁用ip
	updisable:function(data,id,disable){
		myFn.invoke({
			url:request('/console/updisable'),
			data:{
				sid:id,
				disable:disable
			},
